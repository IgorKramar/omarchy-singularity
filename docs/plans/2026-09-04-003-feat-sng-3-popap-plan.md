...
- **Open blockers:** нет.
- **Product Contract preservation:** changed: R2 — исследование показало, что цель IPC `singularity` уже занята сервисом, поэтому «своя цель» уточнена до отдельного имени. Остальные R и все идентификаторы без изменений.
...
- R2. У попапа есть отдельная цель IPC под собственным именем, не совпадающим с целью сервиса; обслуживается она единственным обработчиком в виджете бара. Через неё проходят только команды управления окном — открыть, закрыть, переключить; содержимое кэша, названия задач и проектов она не отдаёт.
...
- KTD1. **Попап — вложенная панель виджета бара, а не отдельный вид плагина.** Манифест уже объявляет `overlay`, и второй загрузочный вид с ним столкнётся; штатные `plugins/panels/clock` и `weather` собраны ровно так — виджет держит `Loader` и впрыскивает в панель `bar`, `settings`, `anchorItem` и ссылку на себя. Governs R1, R3.
- KTD2. **Цель IPC попапа называется `singularity.popup` и живёт в виджете бара.** Имя `singularity` занято обработчиком сервиса, а сервис — один на оболочку; второй обработчик на той же цели оболочка отвергнет. Штатный `summon` по идентификатору плагина сюда не годится: он уходит в загрузчик оверлея, потому что манифест объявляет `overlay`. Governs R2.
- KTD3. **Команда открытия доходит до экземпляра сфокусированного монитора, а не до всех.** Фиксированную цель занимает один экземпляр из нескольких, поэтому обработчик не открывает попап у себя, а просит бар выбрать слот — тем же механизмом, которым оболочка водит панельные хоткеи. `broadcast()` остаётся для команд, которые действительно нужны всем экземплярам. Governs R3.
- KTD4. **Курсор держится тремя свойствами: позиция, идентификатор задачи и признак «ведёт клавиатура».** После перестроения списка позиция ищется по идентификатору, с откатом на прежний индекс и снятием курсора, если не нашлось ни того ни другого. Признак «ведёт клавиатура» не даёт покоящемуся указателю перехватить выделение. Приём взят из плагина `mozzy.kaj`, где он уже работает. Governs R20, R22.
- KTD5. **Разрез по вкладке, группировка, скрытые секции и помощники курсора — чистые функции в `Api.mjs`.** Инстанцирует KD8. Каждая возвращает прежнюю ссылку на неизменном входе; в QML остаются присваивание и сравнение скаляров. Governs R5, R6, R13, R22.
- KTD6. **Список — `Flickable` с `Repeater`, а не `ListView`.** Секции вложены как повторитель в повторителе, прокрутка следует за курсором вручную. `ListView` потребовал бы плоской модели с заголовком внутри делегата, что мешает сворачиванию секций. Governs R6, R7.
- KTD7. **Пустые состояния собираются текстом с вычисленной причиной.** Готового компонента для них в `Ui/` нет ни у одного плагина оболочки. Governs R24, R25, R26.
- KTD8. **Сдвоенный `Enter` гасится флагом.** Перехватчик клавиш поднимает возврат и активацию подряд; обработчик возврата взводит флаг, обработчик активации его снимает и выходит. Приём из плагина todoist. Governs R17.

### High-Level Technical Design

```mermaid
flowchart TB
  subgraph bar["BarWidget.qml — один экземпляр на монитор"]
    PILL["пилюля SNG-2<br/>Ui/BarWidget: vertical, broadcast()"]
    LOADER["Loader → Panel.qml"]
    IPC["IpcHandler singularity.popup<br/>занимает один экземпляр"]
  end
  subgraph panel["Panel.qml — Ui/Panel, manageIpc: false"]
    KP["KeyboardPanel + PanelKeyCatcher"]
    LIST["Flickable + Repeater<br/>секции → строки"]
  end
  SVC["Service.qml<br/>allTasks · tasks · projects · filterApplied"]
  API["Api.mjs — чистые функции"]

  PILL -->|клик| LOADER
  IPC -->|выбор слота по фокусу| LOADER
  LOADER --> panel
  SVC --> API
  API -->|"вид: секции, счётчики, курсор"| LIST
  KP --> LIST
```

Панель получает от виджета `bar`, `settings`, якорь и ссылку на виджет-хозяин: последняя нужна, потому что бар ищет слот по виджету, а не по вложенной панели, — без неё `Tab` молча не сработает.

---

## Implementation Units

### U1. Чистые функции вида и курсора в `Api.mjs`

- **Goal:** разрез по вкладке, группировка по проектам, скрытые секции и помощники курсора — всё, что определяет содержимое попапа, покрыто `node --test`.
- **Requirements:** R5, R6, R8, R10, R13, R22; KD8, KTD5, KTD4.
- **Dependencies:** нет.
- **Files:** `Api.mjs` (изменить), `test/api.test.mjs` (изменить).
- **Approach:**
  1. `sliceByTab(tasks, tab, now)` — «сегодня», «просрочено», «все»; просрочка считается `isOverdue(task, now)`, а не читается из замороженного признака.
  2. `groupByProject(tasks, projects)` — секции с названием и числом; задачи без проекта и задачи с неразрешимым `projectId` идут в две **разные** секции.
  3. `hiddenGroups(allTasks, tasks, projects)` — секции скрытых проектов из разности наборов, плюс число скрытых задач.
  4. `flattenGroups(groups)` — плоская последовательность, по которой ходит курсор: заголовки и строки вперемешку, в порядке отрисовки.
  5. `moveCursor(index, delta, length)` и `cursorIndexForId(list, id, fallbackIndex)` — перенос курсора и его восстановление после перестроения: сперва по идентификатору, затем откатом на прежний индекс, иначе `-1`.
  6. Каждая функция, возвращающая набор, отдаёт **прежнюю ссылку** на неизменном входе — тем же приёмом, что `computeView`.
- **Patterns to follow:** существующие `computeView`, `applyProjectFilter`, `sameOrder` в `Api.mjs`; форма тестов в `test/api.test.mjs` с фиксированным «сейчас» и построителями `task()`/`body()`.
- **Execution note:** сначала тест на стабильность ссылки для каждой новой функции, возвращающей набор, — именно эта дыра дала P1 в SNG-2, и покрыта она должна быть до того, как появится потребитель.
- **Test scenarios:**
  - Covers R5. Задача со стартом сегодня попадает в «сегодня» и не попадает в «просрочено»; при `now` следующих суток — наоборот, без изменения самой задачи.
  - Covers R5. Разрез «все» по длине равен сумме «сегодня» и «просрочено».
  - Covers R6, R8. Задачи трёх проектов дают три секции с верными числами; задачи без проекта — отдельная секция.
  - Covers R10. Задача с `projectId`, которого нет в кэше проектов, идёт в секцию «неизвестный проект», а не сливается с секцией «без проекта».
  - Covers R13. Разность полного окна и отсеянного вида даёт секции скрытых проектов и верное число скрытых **задач**, а не число названий в настройке.
  - Covers R13. При пустом списке исключаемых скрытых секций нет и число скрытых равно нулю.
  - Повторный вызов каждой функции на неизменном входе возвращает ту же ссылку; изменение входа ссылку меняет.
  - Covers R22. `cursorIndexForId` находит прежнюю задачу после вставки строки выше неё; при исчезновении задачи откатывается на прежний индекс; при пустом списке возвращает `-1`.
  - Covers R22. `moveCursor` заворачивается по кругу на обоих концах.
- **Verification:** `TZ=Asia/Omsk node --test test/api.test.mjs` зелёный, число тестов выросло с 32.

### U2. Каркас попапа и его подключение к пилюле

- **Goal:** попап открывается кликом и командой, закрывается `Esc`, живёт на своём мониторе и не ломает пилюлю SNG-2.
- **Requirements:** R1, R2, R3, R18, R20, R21; KTD1, KTD2, KTD3.
- **Dependencies:** U1.
- **Files:** `Panel.qml` (создать), `BarWidget.qml` (изменить).
- **Approach:**
  1. `Panel.qml` на базовом типе `Ui/Panel`: `manageIpc: false`, свойство со ссылкой на виджет-хозяин, переопределённый переход на соседнюю панель, передающий эту ссылку.
  2. `BarWidget.qml` остаётся на `Ui/BarWidget` — `vertical`, `barSize` и `broadcast()` сохраняются. Добавляются `Loader`, впрыск (`bar`, `settings`, якорь, ссылка на себя) с повторным вызовом через `Qt.callLater`, и утиный контракт: `opened`, `open()`, `close()`, `togglePanel()`, `closeForPopoutSwitch()`.
  3. Левый клик по пилюле открывает попап вместо нынешнего бездействия; средняя кнопка сохраняет обновление.
  4. Единственный `IpcHandler` на цели `singularity.popup` в виджете бара; открытие идёт в экземпляр сфокусированного монитора.
  5. Открытие запускает опрос, если с последней синхронизации прошло больше порога свежести.
- **Patterns to follow:** `plugins/panels/clock/BarWidget.qml` и `plugins/panels/clock/Panel.qml` в оболочке — впрыск, утиный контракт, ссылка на виджет-хозяин, переопределённый переход.
- **Test scenarios:** (живая проверка в оболочке — QML вне `node --test`)
  - Covers R1. Клик открывает попап, повторный клик и `Esc` закрывают.
  - Covers R3. На двух мониторах попап открывается на том экране, по которому кликнули.
  - Covers R2, R3. `omarchy-shell singularity.popup toggle` открывает попап на сфокусированном мониторе, а не на произвольном.
  - Covers R18. `Tab` уводит на соседнюю панель бара и возвращается по кругу.
  - Пилюля не потеряла поведение SNG-2: в вертикальном баре число по-прежнему уходит в подсказку.
  - В журнале оболочки нет предупреждения о столкновении целей IPC.
- **Verification:** попап открывается всеми тремя способами; журнал оболочки без ошибок загрузки.

### U3. Список: вкладки, секции, строки, прокрутка

- **Goal:** содержимое попапа — то, что вернули функции U1, с сворачиваемыми секциями и прокруткой, следующей за курсором.
- **Requirements:** R4, R6, R7, R8, R9, R10, R11, R12, R27, R28; KTD6, KD5.
- **Dependencies:** U1, U2.
- **Files:** `Panel.qml` (изменить), `TaskRow.qml` (создать).
- **Approach:**
  1. Сегмент вкладок над списком; активная вкладка — свойство панели, содержимое берётся разрезом из U1.
  2. `Flickable` с `Repeater` по секциям и вложенным `Repeater` по строкам; высота содержимого от колонки, прокрутка активна только когда содержимое выше окна.
  3. Свёрнутость хранится набором идентификаторов проектов, а не индексов, — переживает и переключение вкладок, и отсутствие секции на соседней вкладке.
  4. Строка задачи: название, метка дедлайна, признак просрочки, приоритет цветом **и** коротким текстовым обозначением. Все строки из API выводятся простым текстом.
  5. Скрытая секция отрисовывается приглушённой и разворачивается тем же обработчиком, что обычная.
  6. `ensureVisible` для строки под курсором — пересчёт положения относительно содержимого, как в `mozzy.kaj`.
- **Patterns to follow:** `mozzy.kaj/Panel.qml` — `Flickable` с повторителями, ручной `ensureVisible`, разворачивание секции; `Ui/PanelSectionHeader` и `Ui/PanelSeparator` для заголовков и разделителей; ширина и высота через `fittedContentWidth`/`fittedContentHeight` базовой панели.
- **Test scenarios:** (живая проверка)
  - Covers R4, R7. Переключение вкладки не роняет свёрнутость секций.
  - Covers R7. Секция, свёрнутая на одной вкладке и отсутствующая на другой, остаётся свёрнутой при возврате.
  - Covers R6, R10. Три секции с верными числами; задача с неразрешимым проектом — в отдельной секции.
  - Covers R11, R12. Скрытая секция отличима на вид и разворачивается.
  - Covers R28. Задача с названием, содержащим разметку, показывается как текст, а не разбирается.
  - Covers R27. После смены темы цвета попапа меняются вместе с оболочкой.
  - Список длиннее окна прокручивается, короткий — нет.
- **Verification:** глазами в оболочке на временных задачах; журнал без ошибок.

### U4. Клавиатура и указатель

- **Goal:** попап полностью управляется с клавиатуры, а покоящийся указатель не перехватывает выделение.
- **Requirements:** R15, R16, R17, R19, R22; KTD4, KTD8, KD6.
- **Dependencies:** U3.
- **Files:** `Panel.qml` (изменить), `TaskRow.qml` (изменить).
- **Approach:**
  1. `PanelKeyCatcher` внутри `KeyboardPanel`: вертикальное движение водит курсор по плоской последовательности из U1, горизонтальное переключает вкладку, `Esc` закрывает, `Tab` уходит на соседнюю панель.
  2. Активация на заголовке сворачивает и разворачивает секцию; на строке задачи не делает ничего.
  3. Сдвоенный `Enter` гасится флагом: обработчик возврата взводит, обработчик активации снимает и выходит.
  4. Курсор — три свойства из KTD4; перестроение списка обрабатывается вызовом `cursorIndexForId`.
  5. Признак «ведёт клавиатура» гасится при движении указателя; для фильтрации ложных наведений берётся `Ui/PointerMoveGate`.
- **Patterns to follow:** `mozzy.kaj/Panel.qml` — три свойства курсора и обработчик перестроения; `io.github.aryan-techie.todoist/Panel.qml` — гашение сдвоенного `Enter`; `plugins/panels/clock/Panel.qml` — размещение перехватчика внутри `KeyboardPanel`.
- **Test scenarios:** (живая проверка)
  - Covers R15, R19. Стрелками достижимы все строки и все заголовки, без мыши.
  - Covers R16. `h`, `l` и обе горизонтальные стрелки переключают вкладки одинаково.
  - Covers R17. Активация на заголовке сворачивает секцию; на строке задачи ничего не происходит; действие не срабатывает дважды.
  - Covers R22. Пришедший во время навигации ответ не уводит выделение на соседнюю задачу; при исчезновении задачи выделение уходит на следующую в секции.
  - Покоящийся указатель над списком не перехватывает выделение у клавиатуры.
- **Verification:** весь путь «открыть, дойти до задачи, свернуть секцию, сменить вкладку, закрыть» проходится без мыши.

### U5. Состояния и подвал

- **Goal:** попап честен во всех состояниях — включая те, где он мог бы промолчать.
- **Requirements:** R13, R14, R21, R23, R24, R25, R26; KTD7, KD7.
- **Dependencies:** U3.
- **Files:** `Panel.qml` (изменить).
- **Approach:**
  1. Нерабочие состояния сервиса — нет токена, ошибка, загрузка при пустом кэше — своим видом и **нашей** формулировкой причины; текст исключения, тело ответа и вывод `stderr` на экран не идут.
  2. Пустота разбирается по причине: окно пусто; вкладка пуста при непустом окне; всё окно ушло под отсев. Три разные формулировки, «всё чисто» и «скрыто N» вместе не показываются.
  3. Подвал: число скрытых задач, состояние отсева (в том числе «не применён» с названиями, не совпавшими ни с одним проектом), признак идущего обновления и время последней синхронизации.
- **Patterns to follow:** формулировки состояний в `BarWidget.qml` пилюли SNG-2 — попап обязан говорить не меньше; `mozzy.kaj/Panel.qml` — текст пустого состояния с вычисленной причиной.
- **Test scenarios:** (живая проверка)
  - Covers R23. Без файла токена — состояние «нет токена» с указанием, куда его положить.
  - Covers R23. При ошибке запроса на экране наша формулировка, а не текст исключения.
  - Covers R24, R25, R26. Три вида пустоты дают три разных сообщения.
  - Covers R14. При заданном списке исключаемых и ещё не пришедшем кэше проектов подвал говорит, что отсев не применён.
  - Covers R21. Во время обновления это видно, время последней синхронизации показано.
- **Verification:** все состояния воспроизведены и различимы; файл токена возвращён без изменений.

### U6. README

- **Goal:** попап и его клавиатура описаны там, где их прочтут.
- **Requirements:** R16, R17, R19.
- **Dependencies:** U4, U5.
- **Files:** `README.md` (изменить).
- **Approach:** раздел про попап: как открыть тремя способами, полная раскладка клавиш с оговоркой, почему вкладки на горизонтали, а не на `Tab`, и что показывает подвал. Манифест не трогается: попап загружается нашим же виджетом бара, отдельного вида плагина не требует.
...
| --- | --- | --- | --- |
| Тесты модуля | `TZ=Asia/Omsk node --test test/api.test.mjs` | U1 | все зелёные, число тестов выросло с 32 |
| Манифест и файлы | `bash test/omarchy-plugin-validate "$PWD"` | U2, U6 | код возврата 0 |
| CI | джоб `check` на ветке | U1, U6 | зелёный |
| Разбор QML | `qmlformat` по `Panel.qml` и `TaskRow.qml` | U2, U3 | разбираются без ошибок |
| Попап вживую | клик, `Esc`, `omarchy-shell singularity.popup toggle` на двух мониторах | U2 | открывается на своём экране всеми тремя способами |
| Столкновение целей IPC | журнал оболочки после перезапуска | U2 | предупреждения о занятой цели `singularity` нет |
| Список и клавиатура | пройти путь «открыть → дойти до задачи → свернуть секцию → сменить вкладку → закрыть» без мыши | U3, U4 | путь проходится целиком |
| Состояния | нет токена, ошибка, пустое окно, пустая вкладка, всё скрыто отсевом | U5 | пять разных сообщений, ни одно не подменяет другое |
| Тема | `omarchy theme set <другая>` | U3 | цвета попапа меняются вместе с баром |

Живая проверка требует задач в окне: сейчас оно пусто, поэтому проверка идёт на временных задачах в существующем проекте, созданных и удалённых в рамках прогона. Файл токена перед пробой нерабочих состояний копируется и возвращается со сверкой контрольной суммы.

---

## Definition of Done

- Юниты U1–U6 выполнены, каждая строка Verification Contract зелёная, джоб `check` зелёный на ветке `feat/sng-3-popap`.
- Критерии приёмки карточки SNG-3 отмечены; пункт «со звёздочкой» (оптимистичное обновление) относится к мутациям и переносится в SNG-3.1 вместе с ними.
- PR открыт на `main`, в диффе нет временных задач, отладочных строк и локального `shell.json`.
- README описывает попап и полную раскладку клавиш.
