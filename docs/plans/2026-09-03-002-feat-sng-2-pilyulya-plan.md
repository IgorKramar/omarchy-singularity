...
- **Execution note:** регрессию на локальную зону писать первой и убедиться, что она проходит на текущем коде — она фиксирует уже верное поведение, а не чинит его. Прогон требует фиксированной зоны UTC+6 без перехода на летнее время: `TZ=Asia/Omsk node --test`, иначе на раннере в UTC утверждение про 21 сентября ложно и джоб краснеет. Зона исправлена при реализации: изначально ревью закрепило `Asia/Almaty`, но Казахстан перешёл на единый UTC+5 в марте 2024, и на ней утверждение тоже ложно.
- **Test scenarios:**
...
- **Approach:**
  ...
  4. Шагу `node --test` в `.github/workflows/ci.yml` задаётся зона `TZ: Asia/Omsk` — без неё регрессия на смещение падает на раннере в UTC.
...
| --- | --- | --- | --- |
| Тесты модуля | `TZ=Asia/Omsk node --test` | U1 | все зелёные, число тестов выросло с 11 |
