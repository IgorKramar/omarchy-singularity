...
- **Слияние**: заменить по id, затем отбросить всё, что не проходит предикат окна, — один и тот же предикат для обоих путей (`Api.mjs`, функция `isCurrent`; в этом плагине её зовут `merge` для отклика записи и `mergeFull` для опроса).
...
```js
// Так это выглядело в Api.mjs до SNG-7; сегодня инкремента в плагине нет
